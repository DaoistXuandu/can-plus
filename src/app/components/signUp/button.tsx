...
export default function Button({ disabled, handle }: { disabled: boolean, handle: () => void }) {
    return (
        <div className=" w-full mt-16 flex flex-col space-y-2">
            <button
                onClick={handle}
                disabled={disabled} className={`disabled:scale-90 disabled:bg-yellow-200 hover:scale-90 w-full text-center bg-yellow-500 font-bold text-2xl p-3 rounded-xl`}>{submit.button}</button>
